...
const texto1 =
    "En este nivel, tendrás que leer los diálogos de los personajes para entender el contexto de la historia y saber cómo responder más adelante.";
const texto2 =
    "Cuando llegues al final de la escena, aparecerán las posibles respuestas de acuerdo a la situación que se esté experimentando. Sé paciente al responder.";
const texto3 =
    "Si eliges una respuesta incorrecta, perderás una vida y causarás una mala reacción en la situación en la que te encuentres. Si pierdes todas tus vidas, regresarás a la escena 1";
const texto4 =
    "Si respondes correctamente, harás que el mundo cambie y podrás avanzar a la siguiente escena hasta completar el nivel. Luego, podrás pasar al siguiente nivel.";
...
let url = `${Rute}://${dominio}/api/progreso-dos-usuario/${userId}`;
let intentos_2 = 0;
let completada = 0;
...
function sendDataToApi(
    ...
    fetch(`${Rute}://${dominio}/api/progreso-dos-usuario`, options)
        .then((response) => {
            if (!response.ok) {
...
$.ajax({
        ...
        if (sceneResponse.data && sceneResponse.data.length > 0) {
            ...
            sceneResponse.data.forEach(function (escena) {
                ...
                $.ajax({
                    ...
                    url: `${Rute}://${dominio}/api/ppreguntas/`,
                    type: "GET",
                    dataType: "json",
                    success: function (questionResponse) {
                        if (
                            questionResponse.ppreguntas &&
                            questionResponse.ppreguntas.length > 0
                        ) {
                            let preguntasProcesadas = 0; // Contador de preguntas procesadas
                            let totalPreguntas =
                                questionResponse.ppreguntas.filter(
                                    (p) => p.escena_id === escena.id
                                ).length;

                            questionResponse.ppreguntas.forEach(function (
                                pregunta
                            ) {
                                ...
                                if (escena.id === pregunta.escena_id) {
                                    if (pregunta.status == 1) {
                                        ...
                                        $.ajax({
                                            ...
                                            url: `${Rute}://${dominio}/api/prespuestas/`,
                                            type: "GET",
                                            dataType: "json",
                                            success: function (response) {
                                                if (
                                            ...
                                            error: function (
                                                ...
                                                console.error(
                                                    "Error en la solicitud de respuestas:",
                                                    textStatus,
                                                    errorThrown
                                                );
                                            },
                            ...
                            if (
                                escenasProcesadas === sceneResponse.data.length
                            ) {
                                comenzar(); // Llama a comenzar una vez que todo ha terminado
                            }
                        }
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.error(
                            "Error en la solicitud de preguntas:",
                            textStatus,
                            errorThrown
                        );
                    },
    ...
    error: function (jqXHR, textStatus, errorThrown) {
        console.error(
            "Error en la solicitud de escenas:",
            textStatus,
            errorThrown
        );
    },
...
function generarHTMLPorId(idPregunta) {
    ...
    if (contenedorMensajes && preguntaDiccionario) {
        ...
        let elementosPrevios =
            contenedorMensajes.querySelectorAll(".respuesta");
...
function verificarRespuesta(respuestaSeleccionada) {
    let respuestaCorrecta =
        array_opciones[contador][array_opciones[contador].length - 1].correcta;
    let id_pre =
        array_opciones[contador][array_opciones[contador].length - 1].id;
    console.log(id_pre);
            ...
            if (contador + 1 > escenas_.length - 1) {
                ...
                siguiente_escena.onclick = () => {
                    ...
                    modal_title.textContent =
                        array_opciones[contador][0].escena_actual;
                    modal_niveles.style.opacity = "1";
                    modal_niveles.style.visibility = "visible";
                };
            }
...
function actualizarVidas() {
    ...
    const contenedorVidas2 = document.getElementById("vidas-2");
    ...
    contenedorVidas2.innerHTML = "";
    for (let i = 0; i < vidas; i++) {
        const vida = document.createElement("span");
        vida.classList.add("vida");
        vida.textContent = "❤️";
        contenedorVidas2.appendChild(vida);
    }
}
...
function verificarAnchoPantalla() {
    if (execute) {
        if (window.innerWidth > 1024) {
            ...
            document.getElementById("vidas-2").style.display = "none"; // Oculta vidas
            document.getElementById("vidas").style.display = "flex";
        }
    }
}

// Ejecuta la función cada 20 milisegundos
setInterval(verificarAnchoPantalla, 20);
...
function saludos_de_batz_a_junajpu() {
    ...
    change_message(
        document.getElementById("mensaje_batz_a_junajpu_E1_pregregunta"),
        array_opciones[contador][0].pregunta
    );
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.removeEventListener(
            "click",
            saludos_de_batz_a_junajpu
        );
...
function saludo_de_junajpu_a_batz() {
    ...
    seguiente_memsaje.style.pointerEvents = "none";
    change_message(
        document.getElementById("menaje_junajpu_a_batz_E1_respuesta"),
        array_opciones[contador][0].correcta
    );
...
function pregunta_de_batz_E1() {
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.addEventListener(
            "click",
            opciones_para_respuesta_junajpu_E1
        );
    }, 3000);
}
...
function Junajpu_preguinta_a_inchel() {
    seguiente_memsaje.style.pointerEvents = "none";
    change_message(
        document.getElementById("mensaje_junajpu_a_ixchel_E2_pregregunta"),
        array_opciones[contador][0].pregunta
    );
...
function respuesta_ixchel_a_junajpu() {
    ...
    change_message(
        document.getElementById("mensaje_ixchel_a_junajpu_E1_pregregunta"),
        array_opciones[contador][0].correcta
    );
...
function preguta_ixchel_a_junajpu() {
    ...
    change_message(
        document.getElementById("mensaje_ixchel_a_junajpu_E1_pregregunta"),
        array_opciones[contador][1].pregunta
    );
    show(document.getElementById("mensaje_ixchel_a_junajpu"));
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.removeEventListener(
            "click",
            preguta_ixchel_a_junajpu
        );
        seguiente_memsaje.addEventListener(
            "click",
            opciones_para_respuesta_junajpu_E2
        );
    }, 3000);
}
...
function llamada_de_ixchel_a_junajpu() {
    ...
    change_message(
        document.getElementById(
            "mensaje_junajpu_a_ixchel_llamada_telefono_saludo"
        ),
        array_opciones[contador][0].pregunta
    );
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.removeEventListener(
            "click",
            llamada_de_ixchel_a_junajpu
        );
        seguiente_memsaje.addEventListener(
            "click",
            respuesta_de_ixchel_a_junajpu
        );
    }, 3000);
}
...
function respuesta_de_ixchel_a_junajpu() {
    ...
    setTimeout(() => {
        disguise(
            document.getElementById("mensaje_junajpu_a_ixchel_llamada_telefono")
        );
        change_message(
            document.getElementById(
                "mensaje_repuesta_ixchel_a_junajpu_telefono_respuesta_E3"
            ),
            array_opciones[contador][0].correcta
        );
        show(
            document.getElementById(
                "mensaje_repuesta_ixchel_a_junajpu_telefono"
            )
        );
...
function pregunta_de_ixchel_a_junajpu() {
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.addEventListener(
            "click",
            respuestas_de_junajpu_a_ixchel
        );
    }, 3000);
}
...
function escena4() {
    ...
    disguise(document.getElementById("plato_de_sopa"));
    disguise(
        document.getElementById("mensaje_repuesta_ixchel_a_junajpu_telefono")
    );
...
function saludos_de_hijos_a_junajpu() {
    ...
    change_message(
        document.getElementById("mensaje_pregunta_a_junajpu_ixkin_E4"),
        array_opciones[contador][0].pregunta
    );
...
function saludo_padre() {
    ...
    change_message(
        document.getElementById("mensaje_respuestas_junajpu_E4"),
        array_opciones[contador][0].correcta
    );
    ...
    setTimeout(() => {
        show(document.getElementById("junajpu_sonriendo_E4_quieto"));
        disguise(
            document.getElementById("junajpu_cuerpo_completo_E4_hablando")
        );
        seguiente_memsaje.style.display = "flex";
        seguiente_memsaje.style.pointerEvents = "auto";
        ...
        seguiente_memsaje.addEventListener(
            "click",
            niños_le_preguntan_si_pueden_ir
        );
    }, 3000);
}
...
function niños_le_preguntan_si_pueden_ir() {
    seguiente_memsaje.style.pointerEvents = "none";
    disguise(
        document.getElementById("mensaje_pregunta_junajpu_respuestas_hijos")
    );
    disguise(document.getElementById("ixkin_quieta_E4"));
    change_message(
        document.getElementById("mensaje_pregunta_a_junajpu_junam_E4"),
        array_opciones[contador][2].pregunta
    );
    change_message(
        document.getElementById("mensaje_pregunta_a_junajpu_ixkin_E4"),
        array_opciones[contador][2].pregunta
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.addEventListener(
            "click",
            desisicion_junajpu_a_sus_hijos
        );
    }, 3000);
}
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                change_message(
                    document.getElementById(
                        "mensaje_pensamiento_junajpu_repuesta"
                    ),
                    array_opciones[contador][1].correcta
                );
...
function hijos_preguntan_a_ixchel() {
    ...
    change_message(
        document.getElementById("mensaje_ixkin_a_ixcehel_pregunta"),
        array_opciones[contador][0].pregunta
    );
    change_message(
        document.getElementById("mensaje_junam_a_ixcehel_pregunta"),
        array_opciones[contador][0].pregunta
    );
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.removeEventListener(
            "click",
            hijos_preguntan_a_ixchel
        );
        seguiente_memsaje.addEventListener(
            "click",
            ixchel_responde_y_pregunta_a_junajpu
        );
    }, 3000);
}
...
function ixchel_responde_y_pregunta_a_junajpu() {
    ...
    change_message(
        document.getElementById("mensaje_ixchel_a_sus_a_junajpu_pregunta"),
        array_opciones[contador][0].correcta
    );
...
function respuesta_correcta_E5() {
    ...
    change_message(
        document.getElementById(
            "mensaje_junajpu_despues_de_desicion_respuesta"
        ),
        array_opciones[contador][1].correcta
    );
...
function junam_esta_enfermo() {
    seguiente_memsaje.style.pointerEvents = "none";
    change_message(
        document.getElementById("mensaje_de_junam_enfermo_E6_pregunta"),
        array_opciones[contador][0].pregunta
    );
...
function ixchel_responde_a_junam() {
    ...
    change_message(
        document.getElementById("mensaje_de_ixchel_E6_pregunta"),
        array_opciones[contador][0].correcta
    );
...
function junajpu_a_responde_a_ixchel() {
    ...
    change_message(
        document.getElementById("mensaje_de_junajpu_E6_pregunta"),
        array_opciones[contador][1].pregunta
    );
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.addEventListener(
            "click",
            respuestas_de_junajpu_junam_enfermo
        );
    }, 4000);
}
...
function hija_habla_a_mama() {
    seguiente_memsaje.style.pointerEvents = "none";
    change_message(
        document.getElementById("mensaje_ixkin_E7_pregunta"),
        array_opciones[contador][0].pregunta
    );
...
function repuesta_mama() {
    ...
    disguise(document.getElementById("ixchel_quieta_en_la_cocina_E7"));
    change_message(
        document.getElementById("mensaje_ixchel_E7_pregunta"),
        array_opciones[contador][0].correcta
    );
...
function saludos_de_junajpu_a_ixcehl() {
    ...
    change_message(
        document.getElementById("mensaje_ixkin_E8_pregunta"),
        array_opciones[contador][0].pregunta
    );
    ...
    setTimeout(() => {
        ...
        seguiente_memsaje.removeEventListener(
            "click",
            saludos_de_junajpu_a_ixcehl
        );
        seguiente_memsaje.addEventListener(
            "click",
            saludos_de_ixchel_a_junajpu
        );
    }, 3000);
}
...
function saludos_de_ixchel_a_junajpu() {
    ...
    change_message(
        document.getElementById("mensaje_ixchel_E8_pregunta"),
        array_opciones[contador][0].correcta
    );
...
function comenzar() {
    time_teminar = setTimeout(function () {
        ...
        document.getElementById("header_principal").style.display = "flex";
...
document.addEventListener("DOMContentLoaded", function () {
    function deshabilitarOpciones() {
        document
            .querySelectorAll(".mensajes_de_respuestas")
            .forEach((boton) => {
                boton.disabled = true;
            });
    }
